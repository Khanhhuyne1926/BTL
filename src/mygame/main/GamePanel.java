...
import mygame.main.ConfettiManager;
...
public class GamePanel extends JPanel implements Runnable {
    ...
    // BUG FIX: Biến để kiểm soát việc chuyển đổi nhạc, tránh gọi stop/play liên tục trong loop
    ...
    // BUG FIX: Bổ sung các hàm điều khiển âm thanh bị thiếu mà UI gọi tới
    public void setGameMusicVolume(int volume) {
        this.gameMusicVolume = Math.max(0, Math.min(100, volume));
        if (gameMusicVolume > 0) {
            gameMusicMuted = false; // Tự động unmute nếu tăng âm lượng
        }
        refreshAudioSettings();
    }

    public void toggleGameMusicMute() {
        this.gameMusicMuted = !this.gameMusicMuted;
            if (gameMusicMuted) {
    ...
    public void setFootstepVolume(int volume) {
        ...
        if (player != null) player.refreshFootstepVolume();
    }

    public void toggleFootstepMute() {
        this.footstepMuted = !this.footstepMuted;
        if (footstepMuted) {
    ...
    public void refreshAudioSettings() {
        ...
        // Đoạn này lặp lại tension ở code gốc, giữ nguyên nhưng logic đã được bọc an toàn
        if (tensionMusic != null && tensionMusic.isLoaded()) {
            tensionMusic.setVolume(isMusicMuted() ? 0 : getMusicVolume());
        }
    ...
    public void setupGame() {
        ...
        eggsCollected = 0;
        eggEffectTick = 0;
        isTensionPlaying = false; // Reset trạng thái nhạc
    ...
    private void spawnInitialChickens() {
        ...
        switch (currentLevel) {
            case 1:
                //Sang PHẢI: Tăng X (Cộng thêm vào X).
                //Sang TRÁI: Giảm X (Trừ bớt đi ở X).
                //Xuống DƯỚI: Tăng Y (Cộng thêm vào Y).
                //Lên TRÊN: Giảm Y (Trừ bớt đi ở Y).
                
                // KHU VỰC PHÍA TRÊN (TOP)
            chickens.add(new Chicken(this, 198, 128)); // Con gà góc trên bên trái (ngay dưới chỗ lấy trứng)
            chickens.add(new Chicken(this, 480, 64));  // Con gà sát hàng rào phía trên (gần quả bí ngô)
            chickens.add(new Chicken(this, 896, 192)); // Con gà sát mép phải màn hình (phía trên)
            chickens.add(new Chicken(this, 700, 192));
            // KHU VỰC GIỮA (MIDDLE)
            chickens.add(new Chicken(this, 64, 304));  // Con gà sát mép trái (đoạn giữa mê cung)
            chickens.add(new Chicken(this, 512, 384)); // Con gà ngay chính giữa bản đồ (trên thảm cỏ xanh)
            chickens.add(new Chicken(this, 896, 384));

            // KHU VỰC PHÍA DƯỚI (BOTTOM)
            chickens.add(new Chicken(this, 624, 544)); // Con gà ở hốc cụt (gần góc dưới bên phải)
            chickens.add(new Chicken(this, 448, 608)); // Con gà lối đi dưới (gần nhà 
            chickens.add(new Chicken(this, 896, 640)); // Con gà ở góc dưới cùng bên phải màn hình
            chickens.add(new Chicken(this, 700, 635));
            // --- 2 VỊ TRÍ MỚI (TĂNG ĐỘ KHÓ) ---
            chickens.add(new Chicken(this, 74, 60));   // [MỚI] Chặn ngay góc cua sát quả trứng vàng (vừa lấy xong là gặp)
            chickens.add(new Chicken(this, 284, 360)); // [MỚI] Chặn lối đi hẹp ngay trước cổng nhà của Huyne (thử thách cuối cùng)    
    ...
    public void update() {
        ...
        if (gameState == STATE_LEVEL_TRANSITION) {
            if (mouseH.clicked) {
                // Nhấn SẴN SÀNG -> Vào Level 2
                if (ui.readyBtn.contains(mouseH.mouseX, mouseH.mouseY)) {
                    mouseH.resetClick();
                    startLevel2();
                } 
                // Nhấn CHƠI LẠI -> Quay về Level 1
        ...
        if (gameState == STATE_PAUSE) {
            if (mouseH.clicked) {
                // BUG FIX: Sửa lỗi logic nút và dọn dẹp các khối trùng lặp
        ...
        // BUG FIX: Chỉ gọi stop/play nhạc một lần khi trạng thái thay đổi để tránh lag
