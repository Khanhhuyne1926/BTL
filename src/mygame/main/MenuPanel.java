...
public class MenuPanel extends JPanel {
    ...
    private void drawSettingsOverlay(Graphics2D g2) {
        // 1. Phủ một lớp nền tối mờ phía sau để nổi bật khung Settings
        g2.setColor(new Color(0, 0, 0, 180)); // Đen, trong suốt 70%
        g2.fillRect(0, 0, getWidth(), getHeight());

        // 2. Định nghĩa kích thước và vị trí khung Settings (Nhỏ gọn 450x300)
        int panelW = 450;
        int panelH = 320; // Tăng một chút chiều cao để bố cục thoáng hơn
        int panelX = (getWidth() - panelW) / 2;
        int panelY = (getHeight() - panelH) / 2;

        // 3. Vẽ nền khung Settings (Hiệu ứng Gradient màu gỗ)
        GradientPaint baseGp = new GradientPaint(
                panelX, panelY, new Color(90, 60, 30), // Nâu nhạt phía trên
                panelX, panelY + panelH, new Color(50, 30, 15) // Nâu đậm phía dưới
        );
        g2.setPaint(baseGp);
        g2.fillRoundRect(panelX, panelY, panelW, panelH, 30, 30);

        // 4. Vẽ viền nổi cho khung (Tạo cảm giác 3D nhẹ)
        g2.setStroke(new BasicStroke(4f));
        g2.setColor(new Color(255, 230, 180)); // Màu kem sáng
        ...
        // 5. Vẽ Tiêu đề "SETTINGS"
        String title = "SETTINGS";
        g2.setFont(buttonFont.deriveFont(Font.BOLD, 28f)); // Font game lớn

        // Đổ bóng tiêu đề
        g2.setColor(new Color(0, 0, 0, 150));
        drawCenteredText(g2, title, getWidth(), panelY + 48); 
        // Văn bản chính
        g2.setColor(new Color(255, 250, 240)); // Màu trắng ngà
        drawCenteredText(g2, title, getWidth(), panelY + 45);

        // 6. Thiết lập vị trí các thành phần Audio (Sliders, Mute buttons)
        int startY = panelY + 85; // Điểm bắt đầu vẽ các hàng âm thanh
        int rowHeight = 65;      // Chiều cao mỗi hàng (Music, SFX)

        // Căn chỉnh Sliders vào giữa theo chiều ngang
        int sliderW = 200;
        int sliderX = panelX + (panelW - sliderW) / 2 - 20; // Lệch trái một chút để chừa chỗ cho Mute
        int sliderH = 10;

        // Định nghĩa vị trí thực cho nút Mute nhỏ gọn bên phải slider
        int muteBtnW = 60;
        int muteBtnH = 30;
        int muteBtnX = sliderX + sliderW + 15;

        // Cập nhật Bounds để xử lý click chuột
        ...
        // 7. Vẽ các hàng Audio bằng hàm trợ giúp (đã được tinh chỉnh font/màu)
        ...
        // 8. Vẽ nút BACK (Nhỏ gọn, đặt ở dưới cùng)
        int backBtnW = 140;
        int backBtnH = 45;
        // Đặt nút cách mép dưới 25 pixel
        backButton.setBounds((getWidth() - backBtnW) / 2, panelY + panelH - 65, backBtnW, backBtnH);

        // Dùng kiểu vẽ nút game chuẩn để đồng nhất
        drawGameButton(g2, backButton, "BACK", hoveredButton.equals("back"));

        // 9. Cập nhật vị trí "núm" kéo dựa trên âm lượng hiện tại
        updateKnobPositions();
    }

     
    private void drawAudioRowStyle(Graphics2D g2, String label, Rectangle bar, Rectangle knob, Rectangle muteBtn, 
                             float volume, boolean isMuted, boolean isMuteHovered) {
        ...
        // 2. Vẽ thanh Slider (Nền tối)
        g2.setColor(new Color(30, 15, 5, 150)); 
        g2.fillRoundRect(bar.x, bar.y, bar.width, bar.height, 10, 10);

        // 3. Vẽ phần âm lượng đã lấp đầy
        int fillWidth = (int) (bar.width * (volume / 100f));
        if (isMuted) fillWidth = 0; 
        ...
        // --- 4. SỬA LỖI NÚM KÉO (KNOB) ---
        int kSize = 18; // Kích thước cố định cho núm tròn
        int kX = bar.x + fillWidth - (kSize / 2);
        // Tính toán kY để núm nằm chính giữa thanh bar theo chiều dọc
        int kY = bar.y + (bar.height / 2) - (kSize / 2);

        // Cập nhật lại đối tượng knob để xử lý va chạm chuột chính xác
        ...
        // Vẽ núm 3D
        GradientPaint knobGp = new GradientPaint(kX, kY, new Color(255, 250, 200), kX + kSize, kY + kSize, new Color(230, 180, 50));
        g2.setPaint(knobGp);
        ...
        // 5. Vẽ nút Bật/Tắt
        String muteText = isMuted ? "ON" : "OFF";
        Color btnColor = isMuted ? new Color(180, 50, 50) : new Color(100, 100, 100);
        if (isMuteHovered) btnColor = btnColor.brighter();

        g2.setColor(btnColor);
        g2.fillRoundRect(muteBtn.x, muteBtn.y, muteBtn.width, muteBtn.height, 10, 10);

        g2.setFont(buttonFont.deriveFont(Font.BOLD, 14f));
        g2.setColor(Color.WHITE);
        FontMetrics fm = g2.getFontMetrics();
        int tx = muteBtn.x + (muteBtn.width - fm.stringWidth(muteText)) / 2;
        int ty = muteBtn.y + ((muteBtn.height - fm.getHeight()) / 2) + fm.getAscent();
        g2.drawString(muteText, tx, ty);
    }
    private void drawCenteredTextStyle(Graphics2D g2, String text, int y) {
    ...
    // Hàm trợ giúp nhỏ để vẽ text giữa một hình chữ nhật (nếu chưa có)
